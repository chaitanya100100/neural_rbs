...
def channel_permute(img):
    # H x W x C to C x H x W and vice versa. Also supports batch dim.
    ...
    if img.shape[3] == 3 or img.shape[3] == 1:
        assert img.shape[1] > 3 and img.shape[2] > 3
        img = np.transpose(img, [0, 3, 1, 2])
    elif img.shape[1] == 3 or img.shape[1] == 1:
        assert img.shape[2] > 3 and img.shape[3] > 3
...
def compose_pred(pred):
    """Get a single RGB image from multiple object RGB images."""
    # pred: B x [N x O x C x H x W]
    # ret: B x N x C x H x W
    ret = []
    
    for i in range(len(pred)):
        N, O, _, h, w = pred[i].shape
        x = pred[i].view(N, O, -1, h, w) #* omsk[i]
        x = x.sum(1)
        ret.append(x)
    ret = torch.stack(ret)
    return ret
